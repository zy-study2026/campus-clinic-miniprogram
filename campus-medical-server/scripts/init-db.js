...
async function main() {
  try {
    const adminPassword = await generatePasswordHash('123456');
    const studentPassword = await generatePasswordHash('123456');
    const doctorPassword = await generatePasswordHash('123456');

    const sqlTemplate = `-- 创建数据库
...
('ADMIN001', '${adminPassword}', '系统管理员', '13800138000', 2, 1, 1, '1980-01-01');

-- 插入示例学生 (密码: 123456)
INSERT INTO \`user\` (\`username\`, \`password\`, \`name\`, \`phone\`, \`role\`, \`is_phone_bound\`, \`gender\`, \`birth_date\`) VALUES
('2024001', '${studentPassword}', '张三', NULL, 0, 0, 0, '2006-05-15');

-- 插入示例医生 (密码: 123456)
INSERT INTO \`user\` (\`username\`, \`password\`, \`name\`, \`phone\`, \`role\`, \`is_phone_bound\`, \`gender\`, \`birth_date\`) VALUES
('DOC001', '${doctorPassword}', '李医生', '13900139000', 1, 0, 1, '1975-03-20');
...
('流感预防小贴士', '近期流感高发，请同学们注意保暖，勤洗手，保持室内通风。如有发烧、咳嗽等症状，请及时就医。');`;
    ...
    const sqlContent = sqlTemplate
      ...
      .replace('${studentPassword}', studentPassword)
      .replace('${doctorPassword}', doctorPassword);

    const outputPath = path.join(__dirname, '../database.sql');
    fs.writeFileSync(outputPath, sqlContent);

    console.log('✅ 数据库初始化SQL文件已生成:', outputPath);
    console.log('\n示例账号：');
    console.log('管理员: username=ADMIN001, password=123456');
    console.log('学生: username=2024001, password=123456');
    console.log('医生: username=DOC001, password=123456');
